...
from auditor import dispatcher, SUDO_USERS, SUPPORT_USERS, LOGGER
from auditor.modules.disable import DisableAbleCommandHandler, DisableAbleMessageHandler

import auditor.modules.helper_funcs.fun_strings as fun
...
def ports_bug(update, context):
    ...
    if bug:
        context.bot.sendMessage(-1001352295119, "<b>NEW BUG REPORT!</b>\n\n<b>Submitted by</b>: {}.\n\nDescription: <code>{}</code>.".format(
...
Some dank memes for fun or whatever!
 ...
 × /basketball : send ball
 × /dart : sends dart

Some other regex filters are:
`goodmorning` | `goodnight`.

Auditor will reply random strings accordingly when these words are used!
All regex filters can be disabled incase u don't want... like: `/disable goodmorning`.

*#AUTO_CHAT*
Chatbot utilizes the CoffeeHouse API and allows Bot to talk back making your chat more interactive. 
 - /autochat     : Enables Chatbot mode
 - /stopchat      : Disables Chatbot mode 
 
 note - only group admins will be able to use this command others simply can use in bot's pm
...
DECIDE_HANDLER = DisableAbleMessageHandler(Filters.regex(r"(?i)^auditor\?"), decide, friendly="decide")
