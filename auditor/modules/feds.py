...
from auditor import dispatcher, OWNER_ID, SUDO_USERS, WHITELIST_USERS, MESSAGE_DUMP, LOGGER
from auditor.modules.helper_funcs.chat_status import is_user_admin
from auditor.modules.helper_funcs.extraction import extract_user, extract_unt_fedban, extract_user_fban
...
import auditor.modules.sql.feds_sql as sql

from auditor.modules.helper_funcs.alternate import send_message, typing_action, send_action
	...
	if args:
		if args[0] == 'json':
			...
			with BytesIO(str.encode(backups)) as output:
				output.name = "auditor_fbanned_users.json"
				update.effective_message.reply_document(document=output, filename="auditor_fbanned_users.json",
			...
			for users in getfban:
				...
				backups += "\n"
			with BytesIO(str.encode(backups)) as output:
				output.name = "auditor_fbanned_users.csv"
				update.effective_message.reply_document(document=output, filename="auditor_fbanned_users.csv",
