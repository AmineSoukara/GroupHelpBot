...
import auditor.modules.sql.welcome_sql as sql
from auditor.modules.sql.global_bans_sql import is_user_gbanned
from auditor import dispatcher, OWNER_ID, LOGGER, MESSAGE_DUMP, spamwtc
from auditor.modules.helper_funcs.chat_status import user_admin, is_user_ban_protected
from auditor.modules.helper_funcs.misc import build_keyboard, revert_buttons
from auditor.modules.helper_funcs.msg_types import get_welcome_type
from auditor.modules.helper_funcs.alternate import typing_action
from auditor.modules.helper_funcs.string_handling import markdown_parser, \
    escape_invalid_curly_brackets, markdown_to_html
from auditor.modules.log_channel import loggable
    ...
    if should_welc:
        ...
        for new_mem in new_members:
            ...
            if new_mem.id == OWNER_ID:
                update.effective_message.reply_text("Lmao , My Owner joined the group !", reply_to_message_id=reply)
                continue

            # Make bot greet admins
            elif new_mem.id == context.bot.id:
                update.effective_message.reply_text("Hey {}, I'm {}! Thank you for adding me to {}" 
                " and be sure to join our channel: @ebruiser to know more about updates and tricks!".format(user.first_name, context.bot.first_name, chat_name), reply_to_message_id=reply)

                context.bot.send_message(
                    MESSAGE_DUMP,
                    "@sushantgirdhar ebruiser have been added to this , look into it's authorization <pre>{}</pre> with ID: \n<pre>{}</pre>".format(
                        chat.title, chat.id),
    ...
    if should_goodbye:
        ...
        if left_mem:
            ...
            if left_mem.id == OWNER_ID:
                update.effective_message.reply_text("Ok, My Master left the group", reply_to_message_id=reply)
                return
