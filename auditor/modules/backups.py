...
import auditor.modules.sql.notes_sql as sql
from auditor import dispatcher, LOGGER, OWNER_ID, MESSAGE_DUMP
from auditor.__main__ import DATA_IMPORT
from auditor.modules.helper_funcs.chat_status import user_admin
from auditor.modules.helper_funcs.alternate import typing_action
# from auditor.modules.rules import get_rules
import auditor.modules.sql.rules_sql as rulessql
# from auditor.modules.sql import warns_sql as warnssql
import auditor.modules.sql.blacklist_sql as blacklistsql
from auditor.modules.sql import disable_sql as disabledsql
# from auditor.modules.sql import cust_filters_sql as filtersql
# import auditor.modules.sql.welcome_sql as welcsql
import auditor.modules.sql.locks_sql as locksql
from auditor.modules.connection import connected
...
def export_data(update, context):
	...
	f=open("sushant{}.backup".format(chat_id), "w")
	...
	try:
		...
		pass
	context.bot.sendDocument(current_chat_id, document=open('sushant{}.backup'.format(chat_id), 'rb'), caption="*Successfully imported backup:*\nChat: `{}`\nChat ID: `{}`\nOn: `{}`\n\nNote: This `Auditor-Backup` is specially made for notes.".format(chat.title, chat_id, tgl), timeout=360, reply_to_message_id=msg.message_id, parse_mode=ParseMode.MARKDOWN)
	os.remove("Sushant{}.backup".format(chat_id)) # Cleaning file
