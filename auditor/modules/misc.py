...
from auditor import dispatcher, OWNER_ID, SUDO_USERS, SUPPORT_USERS, WHITELIST_USERS, WALL_API, spamwtc
from auditor.__main__ import STATS, USER_INFO, GDPR
from auditor.modules.disable import DisableAbleCommandHandler
...
from auditor.modules.helper_funcs.alternate import typing_action, send_action
...
def gdpr(update, context):
    ...
    update.effective_message.reply_text("Your personal data has been deleted.\n\nNote that this will not unban "
                                        "you from any chats, as that is telegram data, not auditor data. "
...
def ud(update, context):
    ...
    if not text:
       ...
       msg.reply_text("Bruhhhhh , he is my master . He will kill me,if he sees me commenting upon him")
       return
    try:
...
An "odds and ends" module for small, simple commands which don't really fit anywhere
 ...
 × /news :sends top 10 news ( Use in Bot's PM )
 × /type <text> : repeats the text
 × /google <text> : Search Results from google 
 × /app <app name> : Gives you the direct link of the app  
 × /weebify <text> : fancy weebfied text
 
*#SAY*
 × /milksay <text> : says text
 × /cowsay <text> : says text
 × /cheesesay <text> : says text
 
*#MUSIC_DOWNLOADER*
 × /song <query> :send you the direct song you want in mp3
Note - Only group admins will able to use this command , others can simply use in bot's pm

*#PHONE*
 × /phone +9XXXXXXXXXX :send you the info of the mobile number
Note - Only group admins will able to use this command , others can simply use in bot's pm

*#CARBON*
 × /carbon <text> : Get your own carbon image of the text.
 
 *#REMOVE_BG*
 × /removebg ; reply to the image to remove the background of an image.
